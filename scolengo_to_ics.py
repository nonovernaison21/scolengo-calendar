...
def login_scolengo():
    ...
    if res.returncode != 0:
        raise RuntimeError(f"Échec connexion Scolengo : {res.stderr}")
...
def fetch_range(start_date, end_date, temp_filename="temp.ics"):
    cmd = [
        "npx", "scolengo-cli", "export", "calendar",
...
def fetch_period_recursive(start_date, end_date):
    ...
    if len(events) >= 100:
        ...
        return fetch_period_recursive(start_date, mid_date) + fetch_period_recursive(mid_date + timedelta(days=1), end_date)
    return events
...
def main():
    ...
    print(f"Terminé : {len(unique_events)} événements enregistrés.")
